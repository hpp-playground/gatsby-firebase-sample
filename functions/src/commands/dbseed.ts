...
import serviceAccount from '../gatsby-todos-firebase-adminsdk.json';
import { collectionName } from '../services/todos/constants';
import { Todo } from '../services/todos/models/todo';
...
const uploadSeed = async (collection: string, seedFile: string) => {
  ...
  switch (collection) {
    case collectionName.todos: {
      const docs: Required<Todo>[] =
        records.map((record: Todo) => ({
          ...record,
